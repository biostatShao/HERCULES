...
from hercules.core.utils.OptimizeResult import OptimizeResult


class HerculesGrid(HerculesModel):
    """
    A class to fit the `HerculesModel` model to data using a grid of hyperparameters.
    ...
    The class inherits all the basic attributes from the [HerculesModel][hercules.core.model.HerculesModel.HerculesModel] class.

    !!! seealso "See Also"
        * [HerculesGridSearch][hercules.core.model.gridsearch.HerculesGridSearch.HerculesGridSearch]
        * [HerculesBMA][hercules.core.model.gridsearch.HerculesBMA.HerculesBMA]
    ...
    def __init__(self,
                 gdl,
                 grid,
                 initial_per_snp_prior_column=None,
        ...
        :param kwargs: Additional keyword arguments to pass to the parent `HerculesModel` class.
        """
        ...
        self.initial_per_snp_prior_column = initial_per_snp_prior_column
        self.per_snp_prior_input_type = per_snp_prior_input_type
        self._initial_tau_beta = None
        if self.initial_per_snp_prior_column is not None:
            if 'sigma_epsilon' not in self.fix_params:
                raise ValueError(
                    "A per-SNP tau_beta initialization requires a fixed sigma_epsilon grid so "
                    "that residual-variance initialization remains model-specific."
                )
            self._initial_tau_beta = {}
            for chromosome in self.shapes:
                try:
                    values = self.gdl.sumstats_table[chromosome].get_col(
                        self.initial_per_snp_prior_column
                    )
                except (KeyError, AttributeError) as exc:
                    raise ValueError(
                        f"Summary statistics for chromosome {chromosome} do not "
                        f"contain the configured per-SNP initialization column "
                        f"{self.initial_per_snp_prior_column!r}."
                    ) from exc
                if values is None:
                    raise ValueError(
                        f"Summary statistics for chromosome {chromosome} do not "
                ...
                self._initial_tau_beta[chromosome] = prepare_initial_per_snp_precision(
                    values,
                    input_type=self.per_snp_prior_input_type,
                    column=self.initial_per_snp_prior_column,
                    chromosome=chromosome,
                    float_precision=self.float_precision,
                    order=self.order,
                )
            self.tau_beta = self._copy_initial_tau_beta()
    ...
    def initialize_theta(self, theta_0=None):
        ...
        if self._initial_tau_beta is not None:
            self.tau_beta = self._copy_initial_tau_beta()
    ...
    def _copy_initial_tau_beta(self):
        return {
            chromosome: values.copy(order=self.order)
            for chromosome, values in self._initial_tau_beta.items()
        }
    ...
    def to_theta_table(self):
        ...
        if self.n_models == 1:
            if not isinstance(self.tau_beta, dict):
