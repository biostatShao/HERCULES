import numpy as np
import logging
from .HerculesGrid import HerculesGrid


class HerculesGridSearch(HerculesGrid):
    """
    The `HerculesGridSearch` class is an extension of the `HerculesGrid` class that
    implements grid search for the `HerculesModel` models. The grid search procedure
    ...
    * `pseudo_validation`: The model with the highest pseudo-validation R^2 is selected.
    ...
    def __init__(self,
                 ...
                 **kwargs):
        """
        Initialize the `HerculesGridSearch` model.

        :param gdl: An instance of `GWADataLoader`
        :param grid: An instance of `HyperparameterGrid`
        :param kwargs: Additional keyword arguments to pass to the parent `HerculesGrid` class.
        """
