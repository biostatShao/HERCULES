...
class HyperparameterGrid(object):
    ...
    hyperparameters of the standard `HerculesModel` models. It is designed to
    interface with models that operate on grids of hyperparameters,
    such as `HerculesGridSearch` and `HerculesBMA`. The hyperparameters for
    the standard HerculesModel model are:
    ...
    * `lambda_min`: The extra ridge penalty that compensates for the non-PSD nature of the LD matrix.
    ...
    def __init__(self,
        ...
        Create a hyperparameter grid for the standard HerculesModel model with the
