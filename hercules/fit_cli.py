...
def check_args(args):
    ...
    if args.model == 'HerculesMix':
        if args.n_components < 1:
            raise ValueError("The number of components for the HerculesMix model must be at least 1.")
    ...
    if args.initial_per_snp_prior_column is not None and args.hyp_search not in ('GS', 'BMA'):
        raise ValueError(
            "Per-SNP tau_beta initialization is currently supported by grid search and "
...
def prepare_model(args, verbose=True):
    ...
    from hercules.core.model.HerculesMix import HerculesMix
    ...
    if args.hyp_search == 'EM':
        ...
        # then set it here and pass to HerculesModel:
        if args.fix_sigma_epsilon is not None:
            fix_params = {'sigma_epsilon': args.fix_sigma_epsilon}
        else:
            fix_params = None

        if args.model == 'HerculesModel':
             p_model = partial(HerculesModel,
        ...
        elif args.model == 'HerculesMix':
            p_model = partial(HerculesMix,
                              K=args.n_components,
                              float_precision=args.float_precision,
                              low_memory=not args.use_symmetric_ld,
                              lambda_min=lambda_min,
        ...
        from hercules.core.model.gridsearch.HerculesBMA import HerculesBMA
        ...
        p_model = partial(model_class[args.hyp_search],
                          ...
                          dequantize_on_the_fly=args.dequantize_on_the_fly,
                          threads=args.threads,
                          initial_per_snp_prior_column=args.initial_per_snp_prior_column,
                          per_snp_prior_input_type=args.per_snp_prior_input_type)

    else:

        from hercules.core.model.gridsearch.HyperparameterSearch import BayesOpt

        base_model = partial(HerculesModel,
                             float_precision=args.float_precision,
                             low_memory=not args.use_symmetric_ld,
                             lambda_min=lambda_min,
                             dequantize_on_the_fly=args.dequantize_on_the_fly,
...
def fit_model(model, data_dict, args):
    ...
    import time
    import numpy as np
    from hercules.core.utils.exceptions import OptimizationDivergence
    ...
    # ----------------------------------------------------------
    # If the model uses a grid (e.g. HerculesGridSearch, HerculesBMA), then make sure
...
def main():
    ...
    parser = argparse.ArgumentParser(description="""
        Commandline arguments for fitting HerculesModel models to GWAS summary statistics
    """)
    ...
    parser.add_argument('--custom-sumstats-mapper', dest='custom_sumstats_mapper', type=str,
                        help='A comma-separated string with column name mappings between the custom '
                             'summary statistics format and the standard format expected by magenpy/HerculesModel. '
    ...
    parser.add_argument(
        ...
        dest='initial_per_snp_prior_column',
        type=str,
        help=(
            'Canonical magenpy summary-statistics column containing the per-SNP '
            'variance or precision used to initialize tau_beta. FastGWA P is '
            'exposed as PVAL.'
        ),
    )
    parser.add_argument(
        ...
        default='variance',
        help=(
            'Interpret the configured per-SNP initialization column as variance '
            'or precision. HERCULES converts variance to tau_beta by inversion.'
        ),
    )
    ...
    parser.add_argument('-m', '--model', dest='model', type=str, default='HerculesModel',
                        help='The type of PRS model to fit to the GWAS data',
                        choices={'HerculesModel', 'HerculesMix'})
    ...
    parser.add_argument('--n-components', dest='n_components', type=int, default=3,
                        help='The number of non-null Gaussian mixture components to use with the HerculesMix model '
    ...
    parser.add_argument('--threads', dest='threads', type=int, default=1,
                        help='The number of threads to use in the E-Step of HerculesModel.')
...
#             print("--", key, ":", val)
    ...
    if len(fit_results) > 0:
        ...
        if len(eff_tables) > 0:
            eff_tables.to_csv(output_prefix + '.beta_tilder.gz', sep="\t", index=False)
            # HERCULES historically consumes ``.fit.gz`` while this runner used
            # ``.beta_tilder.gz`` for the same selected posterior table.  Write
            # both byte-equivalent logical outputs; this is an interface alias
            # and does not change model fitting or model selection.
